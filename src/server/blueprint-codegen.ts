...
export function createBlueprintScaffold(request: BlueprintScaffoldRequest): BlueprintScaffold {
  ...
  const runtimeFilesByNodeId = new Map<string, string>();
  const sourceFiles = generatedNodes.map((node) => {
    ...
    runtimeFilesByNodeId.set(node.id, fileName);
    ...
    files: [
      ...sourceFiles,
      {
        ...
        contents: `${JSON.stringify({
          ...
          private: true,
          type: 'module',
          scripts: { start: 'node --experimental-transform-types server.mjs' },
          engines: { node: '>=22.7' },
        }, null, 2)}\n`,
        overwrite: true,
      },
      {
        path: 'server.mjs',
        contents: renderRuntimeServer(blueprintName, request.blueprint, runtimeFilesByNodeId),
        overwrite: true,
      },
      {
...
function renderRuntimeServer(
  blueprintName: string,
  blueprint: BlueprintScaffoldRequest['blueprint'],
  runtimeFilesByNodeId: Map<string, string>,
): string {
  const model = JSON.stringify({
    name: blueprintName,
    nodes: blueprint.nodes.map((node) => {
      const filePath = runtimeFilesByNodeId.get(node.id);
      const runtime = filePath ? runtimeTarget(node, filePath) : undefined;
      return {
        id: node.id,
        label: node.label,
        kind: node.kind,
...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    await exportTrace(request, result).catch((error) => console.error('CODE_ATLAS_TRACE_ERROR', error instanceof Error ? error.message : error));
...
async function executeBlueprint(payload) {
  ...
  while (queue.length && steps.length < blueprint.nodes.length) {
    ...
    try {
      ...
      const filePath = node.filePath ?? current.filePath;
      steps.push({ nodeId: node.id, nodeLabel: node.label, nodeKind: node.kind, status: 'success', durationMs: Date.now() - started, input: current.input, output, ...(filePath ? { filePath } : {}), ...(current.viaEdgeId ? { viaEdgeId: current.viaEdgeId } : {}) });
      for (const edge of outgoing.get(node.id) ?? []) queue.push({ nodeId: edge.target, input: output, filePath, viaEdgeId: edge.id });
    } catch (error) {
      failed = true;
      const filePath = node.filePath ?? current.filePath;
      steps.push({ nodeId: node.id, nodeLabel: node.label, nodeKind: node.kind, status: 'failed', durationMs: Date.now() - started, input: current.input, error: error instanceof Error ? error.message : 'Component failed', ...(filePath ? { filePath } : {}), ...(current.viaEdgeId ? { viaEdgeId: current.viaEdgeId } : {}) });
    }
  }
  return { status: failed ? 'failed' : 'completed', startedAt: new Date().toISOString(), entryNodeId: entry.id, steps, ...(failed ? {} : { output }) };
}

async function exportTrace(request, result) {
  const endpoint = process.env.CODE_ATLAS_OTLP_ENDPOINT;
  const token = process.env.CODE_ATLAS_OTLP_TOKEN;
  if (!endpoint || !token || !result.steps.length) return;
  const traceId = randomBytes(16).toString('hex');
  const base = BigInt(Date.now()) * 1000000n;
  let offsetMs = 0;
  const spans = result.steps.map((step, index) => {
    const durationMs = Math.max(0.001, Number(step.durationMs) || 0.001);
    const start = base + BigInt(Math.round(offsetMs * 1000000));
    const end = start + BigInt(Math.max(1, Math.round(durationMs * 1000000)));
    offsetMs += durationMs;
    const attributes = [
      attribute('code.function.name', step.nodeLabel),
      attribute('code.atlas.blueprint.node.id', step.nodeId),
      attribute('code.atlas.blueprint.node.kind', step.nodeKind),
      ...(step.filePath ? [attribute('code.file.path', step.filePath)] : []),
      ...(index === 0 ? [attribute('http.request.method', request.method ?? 'GET'), attribute('http.route', request.url ?? '/')] : []),
    ];
    const failed = step.status === 'failed';
    return {
      traceId,
      spanId: (index + 1).toString(16).padStart(16, '0'),
      ...(index ? { parentSpanId: index.toString(16).padStart(16, '0') } : {}),
      name: step.nodeLabel,
      kind: index ? 3 : 2,
      startTimeUnixNano: String(start),
      endTimeUnixNano: String(end),
      attributes,
      status: failed ? { code: 2, message: step.error } : { code: 1 },
      events: failed ? [{
        name: 'exception',
        timeUnixNano: String(end),
        attributes: [attribute('exception.type', 'BlueprintRuntimeError'), attribute('exception.message', step.error ?? 'Blueprint component failed')],
      }] : [],
    };
  });
  const response = await fetch(endpoint, {
    method: 'POST',
    headers: { 'content-type': 'application/json', 'x-code-atlas-otlp-token': token },
    body: JSON.stringify({ resourceSpans: [{ resource: { attributes: [attribute('service.name', blueprint.name)] }, scopeSpans: [{ scope: { name: 'code-atlas.blueprint' }, spans }] }] }),
...
async function executeNode(node, input) {
  if (node.runtime?.invocation) return executeSourceNode(node, input);
...
async function executeSourceNode(node, input) {
  const sourceUrl = new URL(node.runtime.filePath, import.meta.url);
  const sourceStats = await stat(sourceUrl);
  sourceUrl.searchParams.set('codeAtlasVersion', String(sourceStats.mtimeMs) + '-' + String(sourceStats.size));
  const sourceModule = await import(sourceUrl.href);
  const exported = sourceModule[node.runtime.exportName];
  if (node.runtime.invocation === 'handler') {
    if (typeof exported !== 'function') throw new Error('Export ' + node.runtime.exportName + ' not found in ' + node.runtime.filePath);
    return await exported({ body: input });
  }
  if (typeof exported !== 'function') throw new Error('Export ' + node.runtime.exportName + ' not found in ' + node.runtime.filePath);
  const instance = new exported();
  if (typeof instance.execute !== 'function') throw new Error('Method execute() not found in ' + node.runtime.exportName);
...
function renderTypeScript(node: BlueprintNode, dependencies: BlueprintNode[], template: BlueprintCodeTemplate): string {
  ...
  const parameters = dependencyNames.map((dependency) => `${lowerFirst(dependency)}: unknown`).join(', ');
  const assignments = dependencyNames.map((dependency) => `    this.${lowerFirst(dependency)} = ${lowerFirst(dependency)};`).join('\n');
  const constructor = dependencyNames.length
    ? `\n${fields}\n\n  constructor(${parameters}) {\n${assignments}\n  }\n`
    : '';
  return `${header}export class ${name} {${constructor}\n  async execute(input: unknown): Promise<unknown> {\n${indent(behavior, 4)}\n  }\n}\n`;
}

function runtimeTarget(node: BlueprintNode, filePath: string): {
  filePath: string;
  exportName?: string;
  invocation?: 'class' | 'handler';
} {
  if (normalizeLanguage(node.language) !== 'typescript') return { filePath };
  const template = node.codegen?.template === 'auto' || !node.codegen?.template
    ? defaultTemplate(node)
    : node.codegen.template;
  if (template === 'interface' || node.kind === 'interface') return { filePath };
  const name = identifier(node.label, 'Component');
  if (template === 'http-handler' || node.kind === 'controller' || node.kind === 'gateway') {
    return { filePath, exportName: `${lowerFirst(name)}Handler`, invocation: 'handler' };
  }
  return { filePath, exportName: name, invocation: 'class' };
}
