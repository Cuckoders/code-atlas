...
- изолированный `worker_thread` для каждого фонового анализа с лимитом памяти и времени: тяжёлый AST-разбор не блокирует HTTP API;
...
Browser / React Flow
        ...
        ▼
Fastify local API ──► background queue ──► isolated worker_thread
        │                         ▲                 │
        │                         └── progress ─────┤
...
1. Отмена активного задания, приоритеты и управляемый пул worker’ов для больших монорепозиториев.
2. Семантический call graph через language servers для Java, Go, Rust, C# и PHP.
3. Kotlin Tree-sitter WASM-адаптер.
